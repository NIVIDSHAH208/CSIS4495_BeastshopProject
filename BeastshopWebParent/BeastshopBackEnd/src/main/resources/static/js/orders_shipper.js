 ...
 */
 ...
 function sendRequestToUpdateOrderStatus(button){
 	...
 	$.ajax({
		...
		updateStstusIconColor(response.orderId, response.status);
	}).fail(function(err){
		showMessageModal("Error: please try updating order again!");
	});
 }
 
 function updateStstusIconColor(orderId, status){
 ...
 function showUpdateConfirmModal(link){
 	...
 	confirmText.text("Are you sure you want to update status of the OrderId: #"+orderId+" to "+status+"?");
